...
const chain = { id: CHAIN_ID, name: `didlab-${CHAIN_ID}`, nativeCurrency:
{ name:"ETH",symbol:"ETH",decimals:18 }, rpcUrls:{ default:{ http:[RPC_URL] } } } as
const;
...
const bMe = await publicClient.readContract({ address: TOKEN, abi,
functionName:"balanceOf", args:[me] }) as bigint;
const bYou= await publicClient.readContract({ address: TOKEN, abi,
functionName:"balanceOf", args:[you] }) as bigint;
console.log(`${label} | Me: ${formatUnits(bMe,18)} CAMP | You: ${formatUnits(bYou,18)}
CAMP`);
};
await bal("Before");
// Transfer 100 CAMP (lower tip)
const tx1 = await wallet.writeContract({
address: TOKEN, abi, functionName: "transfer", args: [you, parseUnits("100", 18n)],
...
address: TOKEN, abi, functionName: "approve", args: [you, parseUnits("50", 18n)],
...
address: TOKEN, abi, functionName: "allowance", args: [me, you]
}) as bigint;
