...
async function main() {
  if (!RPC_URL || !CHAIN_ID || !TOKEN) throw new Error("Missing env (RPC_URL, CHAIN_ID, TOKEN_ADDRESS)");
  ...
  const logs = await publicClient.getLogs({ address: TOKEN, fromBlock, toBlock: "latest" });
